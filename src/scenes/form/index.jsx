    ...
    fetch(url, {
      ...
      .then((data) => {
        if (data.erro) window.alert("CEP não encontrado");
          ...
          setFieldValue("logradouro", data.logradouro).then((response) =>
            console.log(JSON.stringify(response))
          );
        }
      });

        const cep = "06414005"; // CEP da Av. Paulista
        const numero = "277"; // Número do endereço

        console.log("Testando...")

    fetch(`https://api.opencagedata.com/geocode/v1/json?q=${cep}+${numero}&key=bbb38eaa46824cc5acfab9474f90afa1`)
      .then((response) => response.json())
      .then((dados) => {
        // Processa os dados retornados pela API
        const endereco = dados.results[0].formatted;
        const latitude = dados.results[0].geometry.lat;
        const longitude = dados.results[0].geometry.lng;
